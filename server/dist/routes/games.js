...
router.post('/match', authMiddleware, (req, res) => {
    ...
    const stmt = db.prepare('INSERT INTO matches (room_code, mode, winner_username, match_json) VALUES (?, ?, ?, ?)');
...
router.get('/matches/:username', (req, res) => {
    const { username } = req.params;
    const rows = db
        .prepare('SELECT * FROM matches WHERE winner_username = ? OR room_code IN (SELECT code FROM rooms WHERE id IN (SELECT room_id FROM room_players WHERE user_id = (SELECT id FROM users WHERE username = ?))) ORDER BY played_at DESC LIMIT 20')
