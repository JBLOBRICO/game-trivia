...
function generateExtQuestions(): RawQuestion[] {
  ...
  const extraSubjects: RawQuestion[] = [
  ...
  const additionalPack: RawQuestion[] = [];
...
export async function seed() {
  ...
  const stmt = await db.prepare(
    'INSERT INTO questions (category, difficulty, question_text, choices, correct_answer, explanation) VALUES (?, ?, ?, ?, ?, ?)'
  );
