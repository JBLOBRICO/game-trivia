    ...
    io.on('connection', (socket) => {
        ...
        socket.on('game:select_difficulty', async ({ roomCode, difficulty, chosenCategory }) => {
            ...
            try {
                ...
                let question = await db.get('SELECT * FROM questions WHERE category = ? AND difficulty = ? AND id NOT IN (' +
                    Array.from(room.usedQuestionIds).join(',') + ') ORDER BY RANDOM() LIMIT 1', [category, difficulty]);
                // Fallback: If all questions used, allow duplicates
                if (!question) {
                    question = await db.get('SELECT * FROM questions WHERE category = ? AND difficulty = ? ORDER BY RANDOM() LIMIT 1', [category, difficulty]);
                }
                if (!question) {
        ...
        socket.on('game:use_ability', async ({ roomCode }) => {
            ...
            if (player.character === 'The Scholar') {
                ...
                const wrongChoices = room.currentQuestion.choices.filter((c) => c !== correct);
                ...
                try {
                    ...
                    const question = await db.get('SELECT * FROM questions WHERE category = ? AND difficulty = ? ORDER BY RANDOM() LIMIT 1', [category, difficulty]);
                    if (question) {
                        room.currentQuestion = {
                            id: question.id,
                            category: question.category,
...
function startQuestionTimer(roomCode, io) {
    const room = rooms[roomCode];
    if (!room)
        return;
    if (room.questionTimer) {
...
async function processAnswer(roomCode, answer, io) {
    const room = rooms[roomCode];
...
async function handleTileLanding(room, player, tile) {
    let text = '';
    if (tile.type === 'bonus') {
        ...
        const rolls = [
            ...
            { t: 'Tax', act: () => {
                    if (player.shield) {
                        player.shield = false;
                        return 'Shield blocked the Mystery Tax!';
                    }
...
async function saveMatchResults(room) {
    ...
    try {
        ...
        if (room.gameMode !== 'Training') {
            await db.run('INSERT INTO match_history (game_mode, winner_username, players) VALUES (?, ?, ?)', [matchHistoryData.game_mode, matchHistoryData.winner_username, matchHistoryData.players]);
        }
        ...
        if (room.gameMode !== 'Training') {
            for (const p of room.players) {
                if (p.userId) {
                    ...
                    await (0, stats_1.updatePlayerMatchStats)(p.userId, isWinner, correctCount, wrongCount, p.coins, xpEarned);
                }
            }
        }
    }
...
function handlePlayerExit(socketId, io) {
...
function getCleanRoomState(roomCode) {
    const room = rooms[roomCode];
    if (!room)
        return null;
    return {
